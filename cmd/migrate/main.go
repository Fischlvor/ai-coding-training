...
func main() {
	// 命令行参数
	action := flag.String("action", "up", "Migration action: up, down, version, force, drop, status")
	...
	// 获取迁移文件路径
	wd, _ := os.Getwd()
	migrationsPath := filepath.Join(wd, cfg.Migrate.Dir)
...
func showTableStatus(dbURL string) {
	...
	tables := []string{
		"app_row", "environment_row", "config_group_row", "config_item_row", "config_version_row", "release_record_row", "gray_rule_row", "subscription_row", "gray_record_row", "config_snapshot_row", "change_event_row",
	}
